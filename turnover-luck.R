...
to_df |> filter(season == 2024) |> colnames()



# Total "Turnover" Impact

ggplot(to_df |> filter(season == 2024), 
       aes(x = net_to_epa_tot, y = reorder(team, net_to_epa_tot))) +
  labs(
    title = "NFL Drive Killer Luck (2024)",
...
# ggsave("nfl-dk-luck.png", width = 16, height = 12, units = "cm")
